...
### Example: _docker="accetto/argbash-docker"
_docker="TODO"
...
# ARG_POSITIONAL_SINGLE([output],[Output template file (recommended type is '*.m4')])
# ARG_LEFTOVERS([Definitions of the arguments to generate (see 'argbash' documentation)])
# ARG_HELP([Generates a template file in the working directory using an 'argbash' docker container.],[Created container will be automatically removed after use.\nCurrent Docker image to use is \"$_docker\".\nCurrent working directory to use is \"$_workdir\".\nNote that containers must have writing permissions for the working directory.])
...
print_help()
{
  printf '%s\n' "Generates a template file in the working directory using an 'argbash' docker container."
  printf 'Usage: %s [-h|--help] [-v|--version] <output> ... \n' "$0"
  printf '\t%s\n' "<output>: Output template file (recommended type is '*.m4')"
  printf '\t%s\n' "... : Definitions of the arguments to generate (see 'argbash' documentation)"
  printf '\t%s\n' "-h, --help: Prints help"
...
_cmd="docker run -it --rm -e PROGRAM=argbash-init -v ${_workdir}:/work ${_docker} ${_arg_output} ${_leftovers}"

echo "$_cmd"

if ( $_cmd ) ; then
  echo "Template file \"$_arg_output\" has been sucessfully generated into the working directory."
  echo "Process it with 'utility-argbash.sh' to get a usable script."
else
